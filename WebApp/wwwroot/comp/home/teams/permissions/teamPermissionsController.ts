module App {
    ...
    interface ITeamPermissionsControllerScope extends ng.IScope {
        title: string;
        team: Models.IOrgTeam;
        projects: Models.IProject[];
        teamProjects: ITeamProject[];
        userProject: Models.IProject;
    ...
    interface ITeamPermissionsController {
        listType: string;
        activate: () => void;
    }

    class TeamPermissionsController implements ITeamPermissionsController {
        listType: string = '2';

        static $inject: string[] = ["$scope", "$state", "$stateParams", "$q", "orgTeamResource", "projectResource"];
        constructor(
            private $scope: ITeamPermissionsControllerScope,
        ...
        activate() {
            ...
            var orgUserId = this.$stateParams['userId'];

            var teamPromise = this.orgTeamResource.get({ id: teamId }).$promise;
            teamPromise.then((team) => { this.$scope.team = team; });

            var userProjectPromise = this.projectResource.getByUserId({ orgUserId: orgUserId }).$promise;
            userProjectPromise.then((project) => {
                this.$scope.userProject = project;
            });
            ...
            this.$q.all([teamPromise, userProjectPromise, projectsPromise]).then(() => {
                this.$scope.teamProjects = _.filter(this.$scope.teamProjects, (tp) => { return tp.projectId === this.$scope.userProject.id; });
    ...
    angular.module("app").controller("teamPermissionsController", TeamPermissionsController);
}